...
class Solution {
public:
	int createGraph(string& beginWord, string& endWord, unordered_set<string>& wordList) {
		...
		fifo.push(beginWord);
		wordMap[beginWord] = layer;
		while (!fifo.empty()) {
			string word = fifo.front();
			fifo.pop();
			layer = wordMap[word];
			if (word == endWord)
				return layer;
			...
			for (int i = 0; i < wordSize; i++) {
				...
				for (char c = 'a'; c <= 'z'; c++) {
					word[i] = c;
					if (wordList.count(word) && !wordMap.count(word)) {
						fifo.push(word);
						wordMap[word] = layer + 1;
						wordList.erase(word);
					}
				}
				word[i] = ac;
	...
	int ladderLength(string beginWord, string endWord, unordered_set<string>& wordList) {
		return createGraph(beginWord, endWord, wordList);
	}
};
...
void wordLadderMain() {
	...
	unordered_set<string> wordList({ "kid", "tag", "pup", "ail", "tun", "woo", "erg", "luz", "brr",
		...
		"sic", "ted", "wot", "del", "imp", "cob", "way", "ann", "tan", "mci", "job", "wet", "err",
	...
	cout << obj.ladderLength("cet", "ism", wordList);
